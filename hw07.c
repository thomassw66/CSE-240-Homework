...
struct movie* print_by_genre(int type)
{
	...
	struct movie* genre_list = (struct movie*) malloc(sizeof(struct movie*));
	struct movie* last = genre_list;
	struct movie* t = list;
	int flag = 0;
	...
	while (t != NULL){
		if(t->type == type){
			...
			last->actors = t->actors;
			strcpy(last->name, t->name);
			last->rating = t->rating;
			last->type = t->type;
...
int add_actor(char* movie_name, char* actor_name)
{
	...
	while (iter != NULL){
		...
		if(strcmp(iter->name, movie_name) == 0){
			
			struct actor* actors = iter->actors;
			
			if(strcmp(actor_name, actors->name) < 0) {
				struct actor* t = (struct actor*) malloc(sizeof(struct actor*));
				strcpy(t->name, movie_name);
				t->next = actors;
				iter->actors = t;
				return 1;
			}else {
				
				while(actors->next != NULL){
					
					if(strcmp(actors->next, movie_name) < 0){
					...
					else if (strcmp(actors->next, movie_name) == 0){
						return -1;
					}
				}

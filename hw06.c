...
void add(struct movie* new_movie)
{
	if (list == NULL) { // inserting into an empty list
	...
	else if (list->rating <= new_movie->rating) { // insert at beginning of list
			new_movie->next = list;
			list = new_movie;
			return;
	}
		...
		while ( temp->next != NULL ) { // loop through linked list
			if ( temp->next->rating <= new_movie->rating ){ // insert between two nodes
		...
		temp->next = new_movie; // insert at end of list
	}
}
...
struct movie* search(struct movie* movie)
{
	...
	struct movie* temp = list;
	while (temp != NULL) {
		if(strcmp(temp->name, movie->name) == 0){
			return temp;
		}
		temp = temp->next;
	}
...
void delete_one(struct movie* movie)
{
	...
	if(strcmp(movie->name, list->name) == 0) { // delete head of the list
		list = list->next;
		free(temp);
		return;
	}
	while(temp->next != NULL) {
		if(strcmp(temp->next->name, movie->name) == 0) { // delete node in the list
			struct * t = temp->next;

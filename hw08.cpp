...
public:
	...
	float BaseArea()
	{
		...
		switch (temp)
		{
			...
			case Triangular:
				if(side1 != 0 && (side1 < side4 || side1 < side3 || side1 < side2))
					area *= side1;
				if(side2 != 0 && (side2 < side4 || side2 < side3 || side2 < side1))
					area *= side2;
				if(side3 != 0 && (side3 < side4 || side3 < side1 || side3 < side2))
					area *= side3;
				if(side4 != 0 && (side4 < side1 || side4 < side3 || side4 < side2))
					area *= side4;
				area /= 2.0;
				return area;
			case Square:
				return side1 * side1;
			case Rectangular:
				return side1 == side2? (side1 == side3? side1 * side4: side1 * side3) : side1 * side2 ;
		}
	}
...
int main()
{
	...
	std::cout << "Enter the sides of the base of the polhedron:";
	...
	std::cout << "\nThe volume of this prism is: " << prismVolume << std::endl;
